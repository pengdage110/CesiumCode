...
let VideoShed3d = function (viewer, options) {
    this.viewer = viewer;
    ...
    if (!this.cameraPositon || !this.position) {
        console.log("位置坐标错误");
        return;
    }
...
VideoShed3d.prototype.getOptions = function() {
    return this.options;
}

VideoShed3d.prototype.updateOptions = function(options) {
...
VideoShed3d.prototype.initCameraParam = function () {
    let viewPoint = this.CT.enu_to_wgs84({
    ...
    let cameraPosition = Cesium.fromDegrees(this.options.position.x * 1, this.options.position.y + 1, this.options.position.z * 1);
    return {
...
VideoShed3d.prototype.activeVideo = function (url) {
    ...
    this.activeVideoListener || (this.activeVideoListener = function () {
        that.videoTexture && taht.videoTexture.destroy(),
...
VideoShed3d.prototype.createVideoEle = function (url) {
    ...
    //  video.setAttribute("crossorigin", !0);
    //  video.appendChild(source1);
    //  video.appendChild(source2);
    //  video.play();
    //  return video;//动态创建video不知道是不是vue的问题 刷新浏览器有问题
    return document.getElementById("testVideo");
}
...
VideoShed3d.prototype.getOrientation = function () {
    ...
    rightWC = Cesium.Cartesian3.negate(rightWC, cartesian3); //Me:两个空向量取反有何意义？
...
VideoShed3d.prototype.createShadowMap = function () {
    ...
    this.viewShadowMap = new Cesium.ShadowMap({
        ...
        isSpotLight: !0,
        cascadesEnabled: !1,
        context: this.viewer.scene.context,  //这是什么东西？
...
VideoShed3d.prototype.addCameraFrustum = function () {
    this.cameraFrustum = new Cesium.Primitive({
        geometryInstances: new Cesium.GeometryInstances({
